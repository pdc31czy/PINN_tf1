...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, x1, layers, dt, lb, ub, q, t0):  #########3
        ...
        self.U0_pred = self.net_U0(self.x0_tf, self.t0)  # N x (q+1)   ###########
    ...
    def neural_net(self, X, weights, biases):
        ...
        for l in range(0, num_layers - 2):
            W = weights[l]
            print('\n', l + 1, 'layer neural_net W = weights[l]: \n', W)
            b = biases[l]
            print('\n', l + 1, 'layer neural_net b = biases[l]: \n', b)
            H = tf.tanh(tf.add(tf.matmul(H, W), b))
            print('\n', l + 1, 'layer neural_net H=tanh(H*W+b) \n', H)

        W = weights[-1]
        print('\n neural_net W = weights[-l]: \n', W)
        b = biases[-1]
        print('\n neural_net b = biases[-l]: \n', b)
    ...
    def net_U0(self, x, t):  ######
        ...
        f = np.exp(-t) * tf.math.sin(2 * np.pi * x) * (4 * np.pi * np.pi - 1)  #######
        F = U_xx + f  ######
    ...
    def train(self, nIter):
        ...
        start_time = time.time()  ###时间开始
        for it in range(nIter):
            self.sess.run(self.train_op_Adam, tf_dict)

            # Print #####self.weights, self.biases
            if it % 1 == 0:
                elapsed = time.time() - start_time  ####算时间
                loss_value = self.sess.run(self.loss, tf_dict)
                # weights_value = self.sess.run(self.weights, tf_dict)###
                # biases_value = self.sess.run(self.biases, tf_dict)####
                # print('Iterations: %d, Loss: %.3e, W: %.3e, b: %.3e,Time: %.2f' %
                #      (it, loss_value, weights_value, biases_value, elapsed)) #########
                print('Iterations: %d, Loss: %.3e, Time: %.2f' %
                      (it, loss_value, elapsed))
                start_time = time.time()  ###时间结束
...
if __name__ == "__main__":
    ...
    data = scipy.io.loadmat('Data/CNdata.mat')

    t = data['tt'].flatten()[:, None]  # T x 1
    x = data['x'].flatten()[:, None]  # N x 1
    Exact = np.real(data['uu']).T  # T x N
    uExact = np.real(data['uuExact']).T  ##########direct exact data

    idx_t0 = 1  ######t=0.1s
    idx_t1 = 2  ######t=0.2s
    dt = t[idx_t1] - t[idx_t0]  ######dt=0.1s
    ...
    model = PhysicsInformedNN(x0, u0, x1, layers, dt, lb, ub, q, t0)  ##########
    countnum = 1
    model.train(1)  #####Epoch

    U1_pred = model.predict(x_star)
    endtime = time.time()
    print('Predict time: %e' % (endtime - starttime))

    error = np.linalg.norm(U1_pred[:, -1] - Exact[idx_t1, :], 2) / np.linalg.norm(Exact[idx_t1, :],
                                                                                  2)  ####[:,-1]倒数第一列的所有元素 [idx_t1,:]第idx_t1行所有列
    print('Error: %e' % (error))

    # error2 = abs(U1_pred[:,-1] - Exact[idx_t1,:]) ##########predict data - exact data by crank-nicolson
    # plt.subplot(1,2,1),plt.plot(error2) ##########
    # plt.title("Error:PINN-CrankNicolson")
    # plt.xlabel("x")
    # plt.ylabel("error")

    # error3 = abs(U1_pred[:,-1] - uExact[idx_t1,:]) ########predict data - exact data by direct
    # plt.subplot(1,2,2),plt.plot(error3) #######
    # plt.title("Error:PINN-DirectExact")
    # plt.xlabel("x")
    # plt.ylabel("error")
