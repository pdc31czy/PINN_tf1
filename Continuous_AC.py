...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, tb, X_f, layers, lb, ub):
        ...
        self.optimizer_Adam = tf.train.AdamOptimizer(learning_rate= 1.0e-6)
...
if __name__ == "__main__":     
    ...
    #u0_pred, f0_pred = model.predict(X1_star)
    #xx = [-1, -0.5, 0.0, 0.5, 1.0]
    #plt.xticks(range(len(xx)), xx)
    #yy = [-1, -0.5, 0.0, 0.5, 1.0]
    #plt.yticks(range(len(yy)), yy)

    # yy = np.linspace(-1.0 ,1.0 , 1)
    # plt.yticks(yy)
    ...
    #U0_pred = griddata(X1_star, u0_pred.flatten(), (X1, T1), method='cubic')

    ######################################################################
    ############################# Plotting ###############################
    ######################################################################    

    # plt.plot(x, U0[:, 0], 'b-', linewidth=2, label='Exact') ########################
    # yy = np.linspace(-1.0 ,1.0 , 1)
    # plt.yticks(yy)
    #plt.plot(x, U0_pred[:,0], 'r--', linewidth=2, label='Prediction')
    ...
    # ax = plt.subplot(gs1[0, 2])
    # ax.plot(x, Exact[:, 0], 'b-', linewidth=2, label='Exact')
    # ax.plot(x, U0_pred[0,:], 'r--', linewidth=2, label='Prediction')  ################
    # ax.set_xlabel('$x$')
    # ax.set_ylabel('$|u0|$')
    # ax.set_title('$t = %.2f$' % (t[0]), fontsize=10)  ##################
    # ax.axis('square')
