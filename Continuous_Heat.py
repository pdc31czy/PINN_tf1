...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, tb, X_f, layers, lb, ub, u_ub, u_x_lb): 
        ...
        _ , self.u_x_ub_pred = self.net_u(self.x_ub_tf, self.t_ub_tf)
        self.f_pred = self.net_f(self.x_f_tf, self.t_f_tf) 
        
        # Loss
        self.loss = tf.reduce_mean(tf.square(self.u0_tf - self.u0_pred)) + \
                    tf.reduce_mean(tf.square(self.u_lb_pred - self.u_ub)) + \
                    tf.reduce_mean(tf.square(self.u_x_lb - self.u_x_ub_pred)) + \
        ...
        self.optimizer = tf.contrib.opt.ScipyOptimizerInterface(self.loss, 
                                                                ...
                                                                options = {'maxiter': 50000,
                                                                           ...
                                                                           'gtol': 1e-06})
        #  'ftol' : 1.0 * np.finfo(float).eps  'maxfun': 50000, 'maxiter': 50000,
    ...
    def net_f(self, x, t):
        ...
        #f_rhs = np.exp(-t)*tf.math.sin(2*np.pi*x)*(4*np.pi*np.pi-1) #######
        f = u_t - u_xx - f_rhs  ######## 
...
if __name__ == "__main__":     
    ...
    N_b = 50
    N_f = 200
    layers = [2, 100, 100, 100, 100, 1] #input:x,t output:u
    
    data = scipy.io.loadmat('Data/CNdata.mat') ##########
    
    t = data['tt'].flatten()[:,None] #(1，201)
    x = data['x'].flatten()[:,None] #(1，256)
    ...
    idx_x = np.random.choice(x.shape[0], N0, replace=True) ####replace=False
    x0 = x[idx_x,:]
    u0 = Exact[idx_x,0:1]
    ...
    model = PhysicsInformedNN(x0, u0, tb, X_f, layers, lb, ub, u_ub, u_x_lb)
